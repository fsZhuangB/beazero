...
def addSame(vList, direction):
    '''在列表查找相同且相邻的数字相加，找到符合条件的返回的返回True,否则返回False，同时返回增加的分数

    direction == 'left':从右向左查找，找到相同且相邻的两个数字，左侧数字翻倍，右侧数字置零
    direction == 'right':从左向右查找，找到相同且相邻的两个数字，右侧数字翻倍，左侧数字置零

    '''
    score = 0
    if direction == 'left':
        for i in [0, 1, 2]:
            if vList[i] == vList[i + 1] != 0: # 查看vList返回的数字是否相等，并不应等于零
                vList[i] *= 2  # 进行赋值操作，将该数字乘2
                vList[i + 1] = 0 # 对于右侧数字置0
                score += vList[i]
...
def handle(vList, direction):
    '''处理一行(一列)中的数据，最终得到该行(列)的数字状态值，返回得分
...
def operation(v):
    ...
    if op in ['a', 'A']: # 向左移动
        ...
        print('Invalid input, please enter a character in [W, S, A, D] or the lower')
    ...
    # 按2和4出现的机率为3/1来产生随机数2和4
    num = random.choice([2, 2, 2, 4])
    # 产生随机数k, 上一步产生的2或4将被填充到第k个空白区域
...
init(v)
score = 0
print('请输入： W(上) S(下) A(左) D(右), 敲击回车键输入.')
while True:
    display(v, score)
    result = operation(v)
